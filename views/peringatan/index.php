...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['tambah'])) {
        $result = $peringatan->create($_POST);
        header('Location: ?page=peringatan&msg=' . ($result ? 'added' : 'error'));
        exit;
    } elseif (isset($_POST['edit'])) {
        $result = $peringatan->update($_POST['id_peringatan'], $_POST);
        header('Location: ?page=peringatan&msg=' . ($result ? 'updated' : 'error'));
        exit;
    }
}
if (isset($_GET['hapus'])) {
    $peringatan = new Peringatan();
    $peringatan->delete($_GET['hapus']);
    header('Location: ?page=peringatan&msg=deleted');
    exit;
}

$search = isset($_GET['search']) ? trim($_GET['search']) : '';
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'p.tgl_peringatan';
...
$peringatan = new Peringatan();
$totalData = $peringatan->getTotalCount($search);
...
$data = $peringatan->getData($search, $sort, $order, $limitValue, $offset);
...
if ($showEditModal) {
    $editData = $peringatan->getById((int)$_GET['edit']);
...
if (isset($_GET['msg'])) {
    switch ($_GET['msg']) {
        case 'added':   $msg = 'Data peringatan berhasil ditambahkan!'; $msgType = 'success'; break;
        case 'updated': $msg = 'Data peringatan berhasil diupdate!'; $msgType = 'success'; break;
        case 'deleted': $msg = 'Data peringatan berhasil dihapus!'; $msgType = 'success'; break;
...
<div class="card">
    <h2><i class="fas fa-exclamation-triangle"></i> Manajemen Surat Peringatan (SP)</h2>
    ...
    <div class="toolbar">
        <form method="get" class="search-form">
            ...
            <div class="search-box">
                ...
                <input type="text" name="search" placeholder="Cari nama pegawai atau keterangan SP..." value="<?= htmlspecialchars($search) ?>">
                <button type="submit">Cari</button>
                <?php if ($search): ?>
                    <a href="?page=peringatan" class="reset-btn">Reset</a>
                <?php endif; ?>
            </div>
        </form>
        <div class="filter-add">
            ...
            <a href="?page=peringatan&action=tambah" class="btn-add"><i class="fas fa-plus"></i> Tambah SP</a>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table">
            <thead>
                <tr>
                    <th><a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=p.id_peringatan&order=<?= $sort == 'p.id_peringatan' && $order == 'asc' ? 'desc' : 'asc' ?>&limit=<?= $limit ?>&page_num=<?= $page ?>">ID <?= $sort == 'p.id_peringatan' ? ($order == 'asc' ? '▲' : '▼') : '' ?></a></th>
                    <th><a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=e.nama&order=<?= $sort == 'e.nama' && $order == 'asc' ? 'desc' : 'asc' ?>&limit=<?= $limit ?>&page_num=<?= $page ?>">Pegawai <?= $sort == 'e.nama' ? ($order == 'asc' ? '▲' : '▼') : '' ?></a></th>
                    <th><a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=p.jenis_peringatan&order=<?= $sort == 'p.jenis_peringatan' && $order == 'asc' ? 'desc' : 'asc' ?>&limit=<?= $limit ?>&page_num=<?= $page ?>">Jenis SP <?= $sort == 'p.jenis_peringatan' ? ($order == 'asc' ? '▲' : '▼') : '' ?></a></th>
                    <th><a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=p.tgl_peringatan&order=<?= $sort == 'p.tgl_peringatan' && $order == 'asc' ? 'desc' : 'asc' ?>&limit=<?= $limit ?>&page_num=<?= $page ?>">Tanggal <?= $sort == 'p.tgl_peringatan' ? ($order == 'asc' ? '▲' : '▼') : '' ?></a></th>
            ...
            <tbody>
                <?php if (count($data) > 0): ?>
                    ...
                    <tr>
                        <td><?= $row['id_peringatan'] ?></td>
                        <td><?= htmlspecialchars($row['pegawai_nama']) ?></td>
                        <td>
                            <span class="badge badge-danger"><?= htmlspecialchars($row['jenis']) ?></span>
                        </td>
                        <td><?= date('d/m/Y', strtotime($row['tgl_peringatan']??'')) ?></td>
                        <td><?= htmlspecialchars($row['keterangan'] ?? '') ?></td>
                        <td class="actions">
                            <a href="?page=peringatan&edit=<?= $row['id_peringatan'] ?>&search=<?= urlencode($search) ?>&sort=<?= $sort ?>&order=<?= $order ?>&limit=<?= $limit ?>&page_num=<?= $page ?>" class="btn-edit-action" title="Edit"><i class="fas fa-edit" style="color:#3498db;"></i></a>
                            <a href="#" class="btn-delete" data-id="<?= $row['id_peringatan'] ?>" title="Hapus"><i class="fas fa-trash-alt" style="color:#e74c3c;"></i></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="6" class="text-center">Tidak ada data surat peringatan</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=<?= $sort ?>&order=<?= $order ?>&limit=<?= $limit ?>&page_num=<?= $page-1 ?>" class="page-link">« Prev</a>
        <?php endif; ?>
        <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
            <a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=<?= $sort ?>&order=<?= $order ?>&limit=<?= $limit ?>&page_num=<?= $i ?>" class="page-link <?= $i == $page ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
        <?php if ($page < $totalPages): ?>
            <a href="?page=peringatan&search=<?= urlencode($search) ?>&sort=<?= $sort ?>&order=<?= $order ?>&limit=<?= $limit ?>&page_num=<?= $page+1 ?>" class="page-link">Next »</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
<div id="modalForm" class="modal" style="display: <?= $showAddModal ? 'flex' : 'none' ?>;">
    <div class="modal-content" style="width: 500px;">
        ...
        <h3><i class="fas fa-plus-circle"></i> Tambah Surat Peringatan</h3>
        <form method="post">
            ...
            <div class="form-group">
                ...
                <select name="jenis_peringatan" required>
                    <option value="SP 1">SP 1 (Surat Peringatan Pertama)</option>
                    <option value="SP 2">SP 2 (Surat Peringatan Kedua)</option>
                    <option value="SP 3">SP 3 (Surat Peringatan Ketiga / Terakhir)</option>
                </select>
            </div>
            <div class="form-group">
                <label>Tanggal Peringatan</label>
                <input type="date" name="tgl_peringatan" value="<?= date('Y-m-d') ?>" required>
            </div>
            <div class="form-group">
                <label>Keterangan / Alasan Pelanggaran</label>
                <textarea name="keterangan" rows="4" required placeholder="Tuliskan detail bentuk pelanggaran pegawai..."></textarea>
            </div>
...
<div id="modalEditForm" class="modal" style="display: <?= $showEditModal ? 'flex' : 'none' ?>;">
    <div class="modal-content" style="width: 500px;">
        <span class="close" onclick="closeModal()">&times;</span>
        <h3><i class="fas fa-edit"></i> Edit Surat Peringatan</h3>
        <form method="post">
            <input type="hidden" name="edit" value="1">
            <input type="hidden" name="id_peringatan" value="<?= $editData['id_peringatan'] ?? '' ?>">
            <div class="form-group">
                <label>Pegawai</label>
                <select name="id_pegawai" required>
                    <option value="">Pilih Pegawai</option>
            ...
            <div class="form-group">
                <label>Jenis Peringatan</label>
                <select name="jenis_peringatan" required>
                    <option value="SP 1" <?= (isset($editData['jenis_peringatan']) && $editData['jenis_peringatan'] == 'SP 1') ? 'selected' : '' ?>>SP 1 (Surat Peringatan Pertama)</option>
                    <option value="SP 2" <?= (isset($editData['jenis_peringatan']) && $editData['jenis_peringatan'] == 'SP 2') ? 'selected' : '' ?>>SP 2 (Surat Peringatan Kedua)</option>
                    <option value="SP 3" <?= (isset($editData['jenis_peringatan']) && $editData['jenis_peringatan'] == 'SP 3') ? 'selected' : '' ?>>SP 3 (Surat Peringatan Ketiga / Terakhir)</option>
                </select>
            </div>
            <div class="form-group">
                <label>Tanggal Peringatan</label>
                <input type="date" name="tgl_peringatan" value="<?= $editData['tgl_peringatan'] ?? '' ?>" required>
            </div>
            <div class="form-group">
                <label>Keterangan / Alasan Pelanggaran</label>
...
<div id="confirmModal" class="modal" style="display: none;">
    <div class="modal-content" style="width: 350px;">
        ...
        <p>Apakah Anda yakin ingin menghapus data peringatan ini?</p>
        <div class="form-buttons">
...
.btn-add { background: #e74c3c; border: none; color: white; padding: 0 18px; border-radius: 8px; cursor: pointer; height: 38px; display: inline-flex; align-items: center; gap: 6px; text-decoration: none; font-size: 14px; font-weight: 500; }
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    if (deleteId) {
        window.location.href = '?page=peringatan&hapus=' + deleteId + '&search=<?= urlencode($search) ?>&sort=<?= $sort ?>&order=<?= $order ?>&limit=<?= $limit ?>&page_num=<?= $page ?>';
    }
});
